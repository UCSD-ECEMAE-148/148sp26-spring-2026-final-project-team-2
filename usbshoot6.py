...
def main():
    ...
    # Trigger timing state variables
    ...
    try:
        while True:
            ...
            if ids is not None and len(ids) > 0:
                ...
                # Turn on the flywheel immediately when a tag is detected
                ...
                # ── TRIGGER LOGIC ─────────────────────────────────────────────────
                # If target is centered (both errors hit 0 due to deadzone) and trigger isn't running yet
                if error_x == 0.0 and error_y == 0.0 and not trigger_active:
                ...
                print(f"\r[TRACKING] FPS: {fps:4.1f} | Pan Err: {error_x: .3f} | Tilt Err: {error_y: .3f} | Trig: {trigger_active}", end="")
                ...
                if now - last_detection_time > RETURN_TO_CENTER_TIMEOUT:
                    # Shut down flywheel since we haven't seen a tag in a while
                    kit.servo[FLYWHEEL_CHANNEL].fraction = 0.0
            ...
            # ── TRIGGER TIMER CHECK ───────────────────────────────────────────
            # This handles shutting off the trigger after 0.5s smoothly regardless of tracking state
