...
LOST_TIMEOUT = 2.0  # Holds position AND keeps flywheels hot for 2 seconds

# ── Shooter configuration (UPDATED TO MATCH YOUR MANUAL CODES) ───────────────
FLYWHEEL_CHANNEL   = 3
TRIGGER_CHANNEL    = 4

# Restored your precise pulse width boundaries
...
def main():
    ...
    # Set shooter initialization to match your specifications
    ...
    print("\n=======================================================")
    print("RUNNING RE-TUNED AUTO ENGINE WITH DELAYED FLYWHEEL SPIN DOWN")
    print("=======================================================")
    ...
    try:
        while True:
            ...
            if ids is not None and len(ids) > 0:
                ...
                # Active lock found: Spin up flywheels to your target fraction (0.5)
                kit.servo[FLYWHEEL_CHANNEL].fraction = 0.5
                ...
                # Fire trigger mechanism using your designated fraction value (0.7)
                ...
                print(f"\r[LOCK ON] Pan Err: {error_x: .3f} | Tilt Err: {error_y: .3f} | Flywheel: HOT  ", end="")
            
            else:
                time_since_lost = now - last_detection_time

                # Check if the 2.0 second grace window is running
                if time_since_lost < LOST_TIMEOUT:
                    # FIX: Maintain active flywheel state (0.5) while holding position
                    kit.servo[FLYWHEEL_CHANNEL].fraction = 0.5
                    
                    last_error_x = 0.0
                    last_error_y = 0.0
                    print(f"\r[HOLD POSITION] Tracking Lost. Holding position + flywheels hot for {LOST_TIMEOUT - time_since_lost:.1f}s...  ", end="")
                else:
                    # FIX: 2 seconds have fully expired. Kill flywheels only when starting the trek home.
                    kit.servo[FLYWHEEL_CHANNEL].fraction = 0.0
                    ...
                    kit.servo[PAN_CHANNEL].angle = pan_servo
                    set_tilt(kit, tilt_servo)
                    print(f"\r[SEARCHING] Return to center home sequence active. Flywheels: OFF              ", end="")

            # Manage automatic trigger reset windows (Back to 0.0 rest state)
            if trigger_active and (time.monotonic() - trigger_start_time >= 0.5):
                kit.servo[TRIGGER_CHANNEL].fraction = 0.0
                trigger_active = False
