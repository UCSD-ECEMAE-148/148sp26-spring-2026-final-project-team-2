...
class MultiCamShooter:
    ...
    def run_loop(self):
        ...
        # Updated ArUco / AprilTag setup for modern OpenCV versions
        tag_dict = cv2.aruco.getPredefinedDictionary(cv2.aruco.DICT_APRILTAG_36h11)
        params = cv2.aruco.DetectorParameters()
        params.cornerRefinementMethod = cv2.aruco.CORNER_REFINE_NONE 
        
        # Instantiate the official detector object
        ...
        try:
            while True:
                ...
                if best_tag is not None:
                    ...
                    if not self.fw_on:
                        try: 
                            self.kit.servo[FLYWHEEL_CHANNEL].fraction = FLYWHEEL_FRACTION
                        except: 
                            pass
                        self.fw_on = True
            ...
            print("\nStopping loop and cleaning up...")
        finally:
            # Safe shutdown: turn off the flywheel when exiting
            try:
                self.kit.servo[FLYWHEEL_CHANNEL].fraction = 0.0
                print("Flywheel shut down successfully.")
            except:
                pass
            cap.release()

if __name__ == "__main__":
    shooter = MultiCamShooter()
    shooter.run_loop()